...
int main(void)
{
    ...
    Matrix* transforms = (Matrix*)RL_CALLOC(MAX_INSTANCES,
    ...
    auto gen_filepath = [](std::string path) {
        return std::string(ASSETSPATH) + path;
    };
    Shader shader = LoadShader(gen_filepath("shaders/instancing.vs").c_str(), gen_filepath("shaders/base.fs").c_str());
    ...
    // Create a defult material with default internal shader for non-instanced
    // mesh drawing
