...
inline float get_screen_height() {
  return static_cast<float>(GetScreenHeight());
}

inline Eigen::Vector2f get_mouse_position() {
...
inline void draw_texture(const Texture2D& tex, Eigen::Vector2f pos,
                         const Color4f& color, float scalex = 1.0f,
                         float scaley = 1.0f) {
  DrawTexture(tex, pos.x() * scalex, get_screen_height() - pos.y() * scaley,
              color.toColor());
}
