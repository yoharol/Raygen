 ...
 * @brief Add new node
 */
void add_node(Eigen::Vector2f               new_pos,
              ...
              Raygen::d2array<bool>&        connect)
{
    ...
    Raygen::parallel_for(count, [&](const int i) {
...
int main()
{
    ...
    Texture2D       circle_tex = Raygen::load_texture2d(std::string(ASSETSPATH) + "circle-16.png");
    Eigen::Vector2f pos_offset(-8.0f / fwidth, 8.0f / fheight);

    for (int i = 0; i < initial_nodes; i++) {
        Eigen::Vector2f new_pos = Eigen::Vector2f(Raygen::random_real(0.3f, 0.7f), Raygen::random_real(0.3f, 0.7f));
    ...
    while (!WindowShouldClose()) {
        {
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                Eigen::Vector2f mouse_pos = Raygen::get_mouse_canvas_position();
            ...
            for (int i = 0; i < count; i++)
                for (int j = i + 1; j < count; j++) {
                    if (connect.get(i, j))
                        Raygen::draw_line(pos[i], pos[j], line_color, 4.0f, fwidth, fheight);
                }
            for (int i = 0; i < count; i++) {
                Raygen::draw_texture(circle_tex, pos[i] + pos_offset, circle_color, fwidth, fheight);
            }
            DrawFPS(100, 100);
            EndDrawing();
        }
